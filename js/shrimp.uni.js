...
async function main() {
  ...
  const stakingTokenAddr = "0xeBA5D22bBeB146392D032A2F74a735d66A32aeE4";
  ...
  const Y_TOKEN = new ethers.Contract(
    stakingTokenAddr,
    ERC20_ABI,
    App.provider
  );
  const YFFI_DAI_BALANCER_POOL = new ethers.Contract(
    balancerPoolTokenAddr,
    BALANCER_POOL_ABI,
    App.provider
  );
...
//   const yamScale = (await YAM_TOKEN.yamsScalingFactor()) / 1e18;
  ...
  _print(`1 ${stakingTokenTicker}  = $${stakingTokenPrice} (FIXED)\n`);
  ...
  _print(
    `                  = ${toDollar(totalStakedYAmount * stakingTokenPrice)}\n`
  );
  _print(
  ...
  _print(
    `                  = ${toDollar(stakedYAmount * stakingTokenPrice)}\n`
  );
  ...
  _print(
    `Claimable Rewards : ${toFixed(
      earnedYFFI,
      4
    )} ${rewardTokenTicker} = $${toFixed(earnedYFFI * rewardTokenPrice, 2)}`
  );
  ...
  _print(
    `Hourly estimate   : ${toFixed(
      ...
      4
    )} ${rewardTokenTicker} = ${toDollar(
      (YFFIWeeklyEstimate / (24 * 7)) * rewardTokenPrice
    )} (out of total ${toFixed(
      weekly_reward / (7 * 24),
      2
    )} ${rewardTokenTicker})`
  );
